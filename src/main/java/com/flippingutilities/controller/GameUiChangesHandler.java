...
public class GameUiChangesHandler {
        ...
        plugin.getClientThread().invokeLater(() ->
        {
            ...
            if (chatInputText.equals("How many do you wish to buy?")) {
                ...
                if (!selectedItem.isPresent()) {
                    ...
                    flippingWidget.update("quantity", selectedItem.get().getRemainingGeLimit());
                }
                ...
                if (offerText.equals("Buy offer")) {
                    if (selectedItem.isPresent() && selectedItem.get().getLatestInstaSell().isPresent()) {
                        flippingWidget.update("setBuyPrice", selectedItem.get().getLatestInstaSell().get().getPrice());
                    }

                } else if (offerText.equals("Sell offer")) {
                    if (selectedItem.isPresent() && selectedItem.get().getLatestInstaBuy().isPresent()) {
                        flippingWidget.update("setSellPrice", selectedItem.get().getLatestInstaBuy().get().getPrice());
                    }
                }
            }
        });
    ...
    public void onWidgetLoaded(WidgetLoaded event)
    {
        ...
        if (event.getGroupId() == GE_HISTORY_TAB_WIDGET_ID && plugin.getFlippingPanel().isItemHighlighted()) {
            deHighlightOffer();
        }
    }
    ...
    private void highlightOffer() {
        ...
        highlightedItem = plugin.viewTradesForCurrentView().stream().filter(item -> item.getItemId() == highlightedItemId && item.getValidFlippingPanelItem()).findFirst();
        plugin.getFlippingPanel().highlightItem(highlightedItem);
    }
